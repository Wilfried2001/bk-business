...
<div class="flex justify-between items-center mb-4">
    ...
    <a href="<?= url('utilisateurs') ?>"
        class="inline-flex items-center font-semibold px-4 py-2 rounded-md text-sm tracking-wide border border-[#0d47a1] text-[#0d47a1] bg-transparent hover:bg-[#eef4ff]">
        ...
        <form action="<?= url('utilisateurs/store') ?>" method="post">
            ...
            <div class="flex flex-wrap -mx-3 gy-3">
                <div class="md:w-1/2 px-3">
                    ...
                    <input type="text" id="nom" name="nom" class="border rounded-md px-3 py-2 text-sm w-full"
                        placeholder="Ex: Jean Dupont" required>
                    <div class="md:w-1/2 px-3">
                        <label for="email" class="app-label"><i data-lucide="mail"></i> Adresse email</label>
                        <input type="email" id="email" name="email" class="border rounded-md px-3 py-2 text-sm w-full"
                            placeholder="Ex: jean@example.com" required>
                    </div>
                    <div class="md:w-1/3 px-3">
                        <label for="role" class="app-label"><i data-lucide="shield"></i> Rôle</label>
                        <select id="role" name="role" class="border rounded-md px-3 py-2 text-sm w-full bg-white"
                            required>
                    ...
                    <div class="md:w-1/3 px-3">
                        ...
                        <input type="password" id="mot_de_passe" name="mot_de_passe"
                            class="border rounded-md px-3 py-2 text-sm w-full" placeholder="Mot de passe initial"
                            required>
                    </div>
                    <div class="md:w-1/3 px-3 flex items-end">
                        <button type="submit"
                            class="inline-flex items-center font-semibold px-4 py-2 rounded-md text-sm tracking-wide bg-gradient-to-r from-[#0d47a1] to-[#1565c0] text-white w-full">
...
<div class="flex justify-between items-center mb-4">
    ...
    <a href="<?= url('utilisateurs') ?>"
        class="inline-flex items-center font-semibold px-4 py-2 rounded-md text-sm tracking-wide border border-gray-300 text-gray-700 bg-transparent hover:bg-gray-50">
        <i data-lucide="arrow-left"></i> Retour à la liste
    </a>
</div>
        ...
        <form action="<?= url('utilisateurs/' . (int)$utilisateur['id_user'] . '/edit') ?>" method="post">
            ...
            <div class="flex flex-wrap -mx-3 gy-3">
                <div class="md:w-1/2 px-3">
                    <label for="nom" class="app-label"><i data-lucide="user"></i> Nom complet</label>
                    <input type="text" id="nom" name="nom" class="border rounded-md px-3 py-2 text-sm w-full"
                        value="<?= e($utilisateur['nom']) ?>" required>
                </div>
                <div class="md:w-1/2 px-3">
                    <label for="email" class="app-label"><i data-lucide="mail"></i> Adresse email</label>
                    <input type="email" id="email" class="border rounded-md px-3 py-2 text-sm w-full"
                        value="<?= e($utilisateur['email']) ?>" disabled>
                ...
                <div class="w-full px-3 text-right">
                    ...
                    <button type="submit"
                        class="inline-flex items-center font-semibold px-4 py-2 rounded-md text-sm tracking-wide bg-gradient-to-r from-[#0d47a1] to-[#1565c0] text-white">
...
<div class="flex justify-between items-center mb-4">
    ...
    <a href="<?= url('utilisateurs/create') ?>"
        class="inline-flex items-center font-semibold px-4 py-2 rounded-md text-sm tracking-wide bg-gradient-to-r from-[#0d47a1] to-[#1565c0] text-white">
        ...
        <div class="overflow-x-auto">
            <table class="app-table table-mobile-details">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="hidden md:table-cell"><small
                                class="text-gray-500">#<?= e($user['id_user']) ?></small></td>
                        <td><strong><span class="truncate" title="<?= e($user['nom']) ?>"
                                    data-toggle="tooltip"><?= e($user['nom']) ?></span></strong></td>
                        <td><span class="truncate" title="<?= e($user['email']) ?>"
                                data-toggle="tooltip"><?= e($user['email']) ?></span></td>
                        ...
                        <td class="hidden md:table-cell"><small class="text-gray-500"><span class="truncate"
                                    title="<?= e(formatDate($user['date_creation'])) ?>"
                                    data-toggle="tooltip"><?= e(formatDate($user['date_creation'])) ?></span></small>
                        </td>
                        <td>
...
<div class="business-dashboard">
    ...
    <section class="kpi-grid">
        ...
        <article class="business-kpi-card accent-volume">
            ...
            <span class="money-trend <?= e($trendClass($moneyTrend)) ?>"><?= e($trendText($moneyTrend)) ?>
                aujourd'hui</span>
        </article>
        <article class="business-kpi-card accent-alert">
            ...
            <span
                class="risk-badge <?= $nbAlertesActives > 0 ? 'danger' : 'success' ?>"><?= e($nbAlertesActives > 0 ? 'Action requise' : 'Services stables') ?></span>
        </article>
        ...
        <article class="business-kpi-card accent-margin">
            ...
            <div class="objective-bar"><span
                    style="width: <?= e(min(100, (int)round((($rentabiliteMois['taux'] ?? 0) / max(1, ($rentabiliteMois['objectif'] ?? 15))) * 100))) ?>%"></span>
            </div>
    ...
    <section class="dashboard-grid-main">
        ...
        <article class="app-card chart-card">
            ...
            <div class="app-card-body">
                <canvas id="commissionsDailyChart"
                    data-chart='<?= e(json_encode($chartCommissionsDaily ?? ['labels' => [], 'data' => []])) ?>'></canvas>
            </div>
        </article>
        <?php else: ?>
        ...
        <aside class="app-card alert-panel">
            ...
            <div class="app-card-body">
                ...
                <div class="empty-state compact"><i data-lucide="shield-check"></i><span>Aucune alerte active.</span>
                </div>
                <?php else: ?>
                <div class="alert-list">
                    ...
                    <div class="alert-card <?= e($alerte['criticite']) ?>">
                        <div>
                            <strong><?= e($alerte['nom_service']) ?> -
                                <?= e(ucfirst(strtolower($alerte['type_solde']))) ?></strong>
                            <span><?= e(formatMontant((float)$alerte['montant_actuel'])) ?> / seuil
                                <?= e(formatMontant((float)$alerte['valeur_seuil'])) ?></span>
        ...
        <article class="app-card balances-card">
            <div class="app-card-header">
                ...
                <div class="table-actions">
                    ...
                    <a href="<?= url('stocks') ?>" class="app-btn app-btn-sm app-btn-secondary"><i
                            data-lucide="eye"></i> Stocks</a>
                </div>
            </div>
                            ...
                            <tr>
                                <td>
                                    <div class="service-cell">
                                        <span
                                            class="service-avatar"><?= e($serviceInitials($service['nom_service'])) ?></span>
                                        <div>
                                ...
                                <td class="text-right">
                                    ...
                                    <div class="stock-level-bar"><span
                                            style="width: <?= e($stockPercent($float)) ?>%"></span></div>
                                </td>
                                <td class="text-right">
                                    <?= $float && $float['valeur_seuil'] !== null ? e(formatMontant((float)$float['valeur_seuil'])) : 'N/A' ?>
                                </td>
                                <td class="text-right">
                                    <?= $caisse ? e(formatMontant((float)$caisse['montant_actuel'])) : '—' ?>
                                    <div class="stock-level-bar caisse"><span
                                            style="width: <?= e($stockPercent($caisse)) ?>%"></span></div>
                                </td>
                                <td class="text-right">
                                    <?= $caisse && $caisse['valeur_seuil'] !== null ? e(formatMontant((float)$caisse['valeur_seuil'])) : 'N/A' ?>
                                </td>
                                <td><span
                                        class="risk-badge <?= $service['en_alerte'] ? 'danger' : 'success' ?>"><?= e($service['en_alerte'] ? 'Alerte' : 'OK') ?></span>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
        ...
        <article class="app-card profit-card">
            ...
            <div class="app-card-body">
                ...
                <div class="profit-row">
                    ...
                    <div><span
                            style="width: <?= e((int)round(((float)$profit['total_commission'] / $maxProfit) * 100)) ?>%"></span>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
                    ...
                    <table class="app-table table-mobile-details">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><span
                                        class="risk-badge success"><?= e(ucfirst(strtolower($tx['statut'] ?? 'validée'))) ?></span>
                                </td>
